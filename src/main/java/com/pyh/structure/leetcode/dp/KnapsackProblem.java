...
public class KnapsackProblem {
    ...
    public static int kpall(int[] wt, int cap) {
        ...
        // 装入背包：这个时候意味着是使用第i中物品，则 子问题 dp[i][j-wt[i-1]]的基础上（i从1开始，数组下标从0开始，所以这里有i-1表示物品的重量），再加入一个i物品，就可以装满背包j容量，
        // 即 dp[i][j]=dp[i][j-wt[i-1]]，这里可以这么理解，我在使用第i种物品的情况下，装出了j-wt[i-1]容量的背包，然后再往背包里装入物品i，不就是dp[i][j]的方法数了嘛
        // 因为求解的是一共有多少中装法，所以这里的式子就是 不装入背包+装入背包 这两个之和，注意与0-1背包中的区别！
        ...
        // 特别需要注意的是 dp[0][0]=1 ,这个表示的是，使用前0中物品装容量为0的背包的解法，虽然没有物品可以选择，但是背包容量有是0，不需要选择物品，所以不选择物品是唯一的解
