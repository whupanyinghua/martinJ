...
public class FindTargetSumWays {
     ...
     *
     * 优化技巧：
     * 观察推导公式，发现计算dp(i,j)的时候只跟上一层dp(i-1,j)与上一层层dp(i-1,j-nums[i-1]) 状态相关，
     * 其实是可以将二位数组优化为一维数组dp[j]，在计算dp[j]的时候，还没有更新dp[j]回去的情况下dp[j]保存的就是上一轮即i-1的结果，此时考虑dp[j]是从左到右，还是从右到左进行计算，我们考虑在动态规划推导公式中，计算dp(j)涉及上一层dp(j-nums[i-1])中，j-nums[i-1]<=j，那么我们就需要从右往左计算，在计算dp(j)的时候dp(j-nums[i-1])已经还是上一轮循环的结果。
     *
