...
public class MakePalindrome {
    ...
    public static int mp(String s) {
        ...
        // 想想是不是要在s[j]右边插入一个s[i]，在s[i]右边插入字符s[j]，需要两次插入操作，这样就是一个回文字符串 s[i]s[j]...s[i+1...j-1]...s[j]s[i]
        // （注意，往s[j]左边插入一个s[i]，在s[i]左边插入字符s[j]也是一样的）s[j]s[i]...s[i+1...j-1]...s[i]s[j]
        ...
        // 我们不能无脑的插入两个字符来推导结果！！！
        // 可以这样思考，
        // 如果 s[i+1...j]本身变成回文字符串的字符数dp[i+1][j]，然后再往j右边插入字符s[i]，此时只需要再插入一次字符的操作就可以满足即dp[i+1][j]+1
        // 如果 s[i...j-1]本身变成回文字符串的字符数dp[i][j-1]，然后在往i左边插入字符s[j]，此时只需要再插入一次字符的操作就可以满足即dp[i][j-1]+1
