...
public class CoinChangeTwo {
     ...
     * 动态规划：
     * 考虑状态方程
     * 变量:金额，因为硬币总类是有限的，硬币个数无限，因此硬币不算变量
     ...
     * 这里这个等式的含义是说，在加入第i个硬币的时候，dp(i,j)的组合应该是由以下结果组成：第i个硬币出现0次+第i个硬币出现1次+第i个硬币出现2次+...+第i个硬币出现k次
     *
     * 数学方式简化方程
     ...
     *
     * 只取j这个维度来看，我们使用变量i做外层循环，在遍历j的时候，要考虑是从左往右还是从右往左遍历
     * 计算dp(i,j)的时候跟本层的dp(i,j-nums[i-1])以及上一层的的dp(i-1,j)相关，
     * 上一层的dp(i-1,j)我们无需考虑，因为在计算本层的j并且还没有覆盖的时候，当前dp[j]中就是上一轮的dp[j]的结果
     * 本层的dp(i,j-nums[i-1])，因为j-nums[i-1]<j，所以为了保证结果的正确性，
     * 那么我们在计算dp(i,j)的时候是要确保dp(i,j-nums[i-1])已经计算过的，那就是dp[j-nums[i-1]]要先于dp[j]计算，
     * 因此选择的遍历方式是从左往右
     *
